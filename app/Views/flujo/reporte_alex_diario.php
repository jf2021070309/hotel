<?php
/**
 * app/Views/flujo/reporte_alex_diario.php
...
<head>
  ...
  <style>
    ...
    /* LA TABLA VERDE (Estilo requerido) */
    .green-table {
      width: 100%;
  ...
  <div class="report-container">
    ...
    <table class="green-table">
      <thead>
        <tr>
          <th>Origen (Turno)</th>
          <th>Detalle de Extracciones / Gastos</th>
          <th class="text-end">Efectivo Neto en Sobre</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>
            ...
            <small class="text-muted d-block ms-1">Ingreso bruto: S/ <?= number_format($reporte['MAÑANA']['PEN'] + (float)str_replace(['(', ')'], '', explode(')', $reporte['MAÑANA']['egresos_detalle'])[0] ?? 0), 2) ?></small>
          </td>
          <td>
          ...
          <td class="amount-cell">
            S/ <?= number_format($totalManana, 2) ?>
          </td>
        </tr>
        <tr class="bg-light-green">
          <td>
            <div class="turn-badge badge-tarde mb-1">TURNO TARDE</div>
            <small class="text-muted d-block ms-1">Ingreso bruto: S/ <?= number_format($reporte['TARDE']['PEN'] + (float)str_replace(['(', ')'], '', explode(')', $reporte['TARDE']['egresos_detalle'])[0] ?? 0), 2) ?></small>
          </td>
          <td>
            <div class="egresos-detalle">
          ...
          <td class="amount-cell">
            S/ <?= number_format($totalTarde, 2) ?>
          </td>
        </tr>
        <tr class="total-row">
          <td colspan="2" class="text-end py-3">TOTAL EFECTIVO A ENTREGAR (SOLES):</td>
          <td class="amount-cell text-primary py-3" style="font-size: 22px;">
    ...
    <!-- OTRAS MONEDAS (SI HUBIERA) -->
    <?php if ($reporte['MAÑANA']['USD'] > 0 || $reporte['TARDE']['USD'] > 0 || $reporte['MAÑANA']['CLP'] > 0 || $reporte['TARDE']['CLP'] > 0): ?>
    <div class="mt-4">
