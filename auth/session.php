...
function iniciarSesion(array $usuario): void {
    ...
    global $pdo;
    if (isset($pdo)) {
        require_once __DIR__ . '/../app/Models/AuditoriaModel.php';
        $audit = new AuditoriaModel($pdo);
        $audit->registrar($usuario['id'], $usuario['nombre'], 'INICIO_SESION', 'SEGURIDAD', "Inicio de sesión exitoso (Trabajador)");
    }
}
...
function cerrarSesion(): void {
    if (isset($_SESSION['auth_id'])) {
        ...
        if (isset($pdo)) {
            require_once __DIR__ . '/../app/Models/AuditoriaModel.php';
            $audit = new AuditoriaModel($pdo);
            $audit->registrar($_SESSION['auth_id'], $_SESSION['auth_nombre'], 'CIERRE_SESION', 'SEGURIDAD', "El usuario cerró su sesión");
        }
    }
